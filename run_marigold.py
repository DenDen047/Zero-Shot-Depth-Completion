...
if "__main__" == __name__:
    ...
    with torch.autocast("cuda", dtype=torch.float32):
        ...
        marigold_depth, pred_uncert = ensemble_depths(depth, **(ensemble_kwargs or {}))
        marigold_depth = (
            marigold_depth.unsqueeze(0).unsqueeze(0).to(torch.float32).cpu()
        )

        torch.save(
            marigold_depth, os.path.join(args.input_root_dir, "marigold_depth.pt")
        )
